...
namespace KlayGE
{
			...
			display_type_(DT_Final)
	{
		...
		for (size_t vpi = 0; vpi < viewports_.size(); ++ vpi)
		{
			...
			pvp.g_buffer_ms = rf.MakeFrameBuffer();
			pvp.g_buffer_ss = rf.MakeFrameBuffer();
#if DEFAULT_DEFERRED == TRIDITIONAL_DEFERRED
			pvp.lighting_fb = rf.MakeFrameBuffer();
#endif
			pvp.vdm_fb = rf.MakeFrameBuffer();
			pvp.shading_ss_fb = rf.MakeFrameBuffer();
			pvp.shading_ms_fb = rf.MakeFrameBuffer();
			pvp.shadowing_fb = rf.MakeFrameBuffer();
			pvp.projective_shadowing_fb = rf.MakeFrameBuffer();
			pvp.reflection_fb = rf.MakeFrameBuffer();
			for (size_t i = 0; i < pvp.merged_shading_ms_fbs.size(); ++ i)
			{
				pvp.merged_shading_ms_fbs[i] = rf.MakeFrameBuffer();
				pvp.merged_depth_ms_fbs[i] = rf.MakeFrameBuffer();
				pvp.merged_shading_ss_fbs[i] = rf.MakeFrameBuffer();
				pvp.merged_depth_ss_fbs[i] = rf.MakeFrameBuffer();
			}
#if DEFAULT_DEFERRED == LIGHT_INDEXED_DEFERRED
			if (cs_cldr_)
			{
				pvp.lighting_mask_ss_fb = rf.MakeFrameBuffer();
				pvp.lighting_mask_ms_fb = rf.MakeFrameBuffer();
		...
		if (cs_cldr_)
		{
			...
			ms_g_buffer_effect_ = SyncLoadRenderEffect("MultiSampleGBuffers.fxml");
		}
		else
		{
			light_batch_ = 32;
		...
		technique_merge_shadings_[2] = dr_effect->TechniqueByName("MergeShadingMSTech");
		technique_merge_shadings_[3] = dr_effect->TechniqueByName("MergeShadingAlphaBlendMSTech");
		...
		technique_merge_depths_[2] = dr_effect->TechniqueByName("MergeDepthMSTech");
		technique_merge_depths_[3] = dr_effect->TechniqueByName("MergeDepthAlphaBlendMSTech");
		technique_copy_shading_depth_[0] = dr_effect->TechniqueByName("CopyShadingDepthTech");
		technique_copy_shading_depth_[1] = dr_effect->TechniqueByName("CopyShadingDepthMSTech");
		technique_copy_depth_[0] = dr_effect->TechniqueByName("CopyDepthTech");
		technique_copy_depth_[1] = dr_effect->TechniqueByName("CopyDepthMSTech");
#if DEFAULT_DEFERRED == LIGHT_INDEXED_DEFERRED
		if (cs_cldr_)
		{
			technique_resolve_g_buffers_ = ms_g_buffer_effect_->TechniqueByName("ResolveGBuffers");
		...
		taa_pp_[0] = SyncLoadPostProcess("TAA.ppml", "Taa");
		taa_pp_[1] = SyncLoadPostProcess("TAA.ppml", "TaaMS");

		sss_blur_pp_[0] = MakeSharedPtr<SSSBlurPP>(false);
		sss_blur_pp_[0]->SetParam(0, 1.0f);
		sss_blur_pp_[0]->SetParam(1, 1.0f);
		sss_blur_pp_[1] = MakeSharedPtr<SSSBlurPP>(true);
		sss_blur_pp_[1]->SetParam(0, 1.0f);
		sss_blur_pp_[1]->SetParam(1, 1.0f);

		translucency_pp_[0] = SyncLoadPostProcess("Translucency.ppml", "Translucency");
		translucency_pp_[0]->SetParam(6, 100.0f);
		translucency_pp_[1] = SyncLoadPostProcess("Translucency.ppml", "TranslucencyMS");
		translucency_pp_[1]->SetParam(6, 100.0f);
		...
		depth_ms_tex_param_ = dr_effect->ParameterByName("depth_ms_tex");
#if DEFAULT_DEFERRED == TRIDITIONAL_DEFERRED
		lighting_tex_param_ = dr_effect->ParameterByName("lighting_tex");
#endif
		shading_tex_param_ = dr_effect->ParameterByName("shading_tex");
		shading_ms_tex_param_ = dr_effect->ParameterByName("shading_ms_tex");
...
#if DEFAULT_DEFERRED == LIGHT_INDEXED_DEFERRED
		g_buffer_ms_rt0_tex_param_ = ms_g_buffer_effect_->ParameterByName("g_buffer_ms_rt0_tex");
		g_buffer_ms_rt1_tex_param_ = ms_g_buffer_effect_->ParameterByName("g_buffer_ms_rt1_tex");
		g_buffer_ms_ds_tex_param_ = ms_g_buffer_effect_->ParameterByName("g_buffer_ms_ds_tex");
		...
		if (cs_cldr_)
		{
			...
			technique_cldr_lighting_mask_[0] = dr_effect->TechniqueByName("ClusteredDRLightingMask");
			technique_cldr_lighting_mask_[1] = dr_effect->TechniqueByName("ClusteredDRLightingMaskMS");
			...
			lighting_mask_ms_tex_param_ = dr_effect->ParameterByName("lighting_mask_ms_tex");
			shading_in_tex_param_ = dr_effect->ParameterByName("shading_in_tex");
			shading_in_ms_tex_param_ = dr_effect->ParameterByName("shading_in_ms_tex");
			...
			copy_pp_[0] = SyncLoadPostProcess("Copy.ppml", "Copy");
			copy_pp_[1] = SyncLoadPostProcess("Copy.ppml", "CopyMS");
		}
		else
		{
	...
	{
		sss_blur_pp_[0]->SetParam(0, strength);
		sss_blur_pp_[1]->SetParam(0, strength);
	}

	void DeferredRenderingLayer::SSSCorrection(float correction)
	{
		sss_blur_pp_[0]->SetParam(1, correction);
		sss_blur_pp_[1]->SetParam(1, correction);
	}
	...
	{
		translucency_pp_[0]->SetParam(6, strength);
		translucency_pp_[1]->SetParam(6, strength);
	}
		...
		uint32_t sample_count, uint32_t sample_quality)
	{
		...
		if (sample_count == 1)
		{
			...
			pvp.g_buffer_ms_ds_tex = pvp.g_buffer_resolved_ds_tex;
		}
		else
		{
			pvp.g_buffer_ms_ds_tex = rf.MakeTexture2D(width, height, 1, 1, EF_D24S8, sample_count, sample_quality,
				EAH_GPU_Read | EAH_GPU_Write);
			pvp.g_buffer_resolved_ds_tex = rf.MakeTexture2D(width, height, 1, 1, EF_D24S8, 1, 0,
				EAH_GPU_Read | EAH_GPU_Write);
		}
		RenderViewPtr ms_ds_view = rf.Make2DDepthStencilRenderView(*pvp.g_buffer_ms_ds_tex, 0, 1, 0);
		RenderViewPtr ss_ds_view = rf.Make2DDepthStencilRenderView(*pvp.g_buffer_resolved_ds_tex, 0, 1, 0);
		...
		{
			pvp.g_buffer_ms_rt0_tex = rf.MakeTexture2D(width, height, 1, 1, fmt8, sample_count, sample_quality,
				EAH_GPU_Read | EAH_GPU_Write);
			pvp.g_buffer_ms_rt1_tex = rf.MakeTexture2D(width, height, 1, 1, fmt8, sample_count, sample_quality,
				EAH_GPU_Read | EAH_GPU_Write);
		}
		...
		{
			pvp.g_buffer_ms_depth_tex = rf.MakeTexture2D(width, height, 1, 1, depth_fmt, sample_count, sample_quality,
				EAH_GPU_Read | EAH_GPU_Write);
		}
		pvp.g_buffer_rt0_backup_tex = rf.MakeTexture2D(width, height, 1, 1, fmt8,
			sample_count, sample_quality, EAH_GPU_Read);
#if DEFAULT_DEFERRED == LIGHT_INDEXED_DEFERRED
		{
		...
		RenderViewPtr g_buffer_ms_rt0_view = rf.Make2DRenderView(*pvp.g_buffer_ms_rt0_tex, 0, 1, 0);
		RenderViewPtr g_buffer_ms_rt1_view = rf.Make2DRenderView(*pvp.g_buffer_ms_rt1_tex, 0, 1, 0);
		RenderViewPtr g_buffer_ss_rt0_view = rf.Make2DRenderView(*pvp.g_buffer_resolved_rt0_tex, 0, 1, 0);
		RenderViewPtr g_buffer_ss_rt1_view = rf.Make2DRenderView(*pvp.g_buffer_resolved_rt1_tex, 0, 1, 0);

		pvp.g_buffer_ms->Attach(FrameBuffer::ATT_Color0, g_buffer_ms_rt0_view);
		pvp.g_buffer_ms->Attach(FrameBuffer::ATT_Color1, g_buffer_ms_rt1_view);
		pvp.g_buffer_ms->Attach(FrameBuffer::ATT_DepthStencil, ms_ds_view);

		pvp.g_buffer_ss->Attach(FrameBuffer::ATT_Color0, g_buffer_ss_rt0_view);
		pvp.g_buffer_ss->Attach(FrameBuffer::ATT_Color1, g_buffer_ss_rt1_view);
		pvp.g_buffer_ss->Attach(FrameBuffer::ATT_DepthStencil, ss_ds_view);
		...
		pvp.reflection_fb->Attach(FrameBuffer::ATT_DepthStencil, rf.Make2DDepthStencilRenderView(
			pvp.reflection_tex->Width(0), pvp.reflection_tex->Height(0), ms_ds_view->Format(), 1, 0));
...
#endif
		pvp.shading_ss_tex = rf.MakeTexture2D(width, height, 1, 1, shading_fmt, 1, 0, hint);
		for (size_t i = 0; i < pvp.merged_shading_ss_texs.size(); ++ i)
		{
			pvp.merged_shading_ss_texs[i] = rf.MakeTexture2D(width, height, 1, 1, fmt, 1, 0, hint);
			pvp.merged_depth_ss_texs[i] = rf.MakeTexture2D(width, height, 1, 1, depth_fmt, 1, 0, EAH_GPU_Read | EAH_GPU_Write);
		}
#if DEFAULT_DEFERRED == LIGHT_INDEXED_DEFERRED
		if (cs_cldr_)
		{
			...
			{
				pvp.temp_shading_ss_tex = rf.MakeTexture2D(width, height, 1, 1, shading_fmt, 1, 0, EAH_GPU_Read | EAH_GPU_Unordered);
				if (sample_count != 1)
				{
					pvp.temp_shading_ms_tex = rf.MakeTexture2D(width, height, 1, 1, shading_fmt, sample_count, sample_quality,
						EAH_GPU_Read | EAH_GPU_Write);
					pvp.temp_shading_ms_fb = rf.MakeFrameBuffer();
					pvp.temp_shading_ms_fb->Attach(FrameBuffer::ATT_Color0, rf.Make2DRenderView(*pvp.temp_shading_ms_tex, 0, 1, 0));
				}
			}
			...
			pvp.lighting_mask_ss_tex = rf.MakeTexture2D(width, height, 1, 1, lighting_mask_fmt, 1, 0,
				EAH_GPU_Read | EAH_GPU_Write);
			pvp.lighting_mask_ss_fb->Attach(FrameBuffer::ATT_Color0, rf.Make2DRenderView(*pvp.lighting_mask_ss_tex, 0, 1, 0));
			pvp.lighting_mask_ss_fb->Attach(FrameBuffer::ATT_DepthStencil, ss_ds_view);

			if (sample_count == 1)
			{
				pvp.lighting_mask_ms_tex = pvp.lighting_mask_ss_tex;
				pvp.shading_ms_tex = pvp.shading_ss_tex;
				for (size_t i = 0; i < pvp.merged_shading_ss_texs.size(); ++i)
				{
					pvp.merged_shading_ms_texs[i] = pvp.merged_shading_ss_texs[i];
					pvp.merged_depth_ms_texs[i] = pvp.merged_depth_ss_texs[i];
				}
			}
			else
			{
				pvp.lighting_mask_ms_tex = rf.MakeTexture2D(width, height, 1, 1, lighting_mask_fmt, sample_count, sample_quality,
					EAH_GPU_Read | EAH_GPU_Write);
				pvp.lighting_mask_ms_fb->Attach(FrameBuffer::ATT_Color0, rf.Make2DRenderView(*pvp.lighting_mask_ms_tex, 0, 1, 0));
				pvp.lighting_mask_ms_fb->Attach(FrameBuffer::ATT_DepthStencil, ms_ds_view);

				pvp.temp_shading_tex_array = rf.MakeTexture2D(width, height, 1, sample_count, shading_fmt, 1, 0,
					EAH_GPU_Read | EAH_GPU_Write | EAH_GPU_Unordered);

				pvp.shading_ms_tex = rf.MakeTexture2D(width, height, 1, 1, shading_fmt, sample_count, sample_quality,
					EAH_GPU_Read | EAH_GPU_Write);
				for (size_t i = 0; i < pvp.merged_shading_ss_texs.size(); ++ i)
				{
					pvp.merged_shading_ms_texs[i] = rf.MakeTexture2D(width, height, 1, 1, fmt, sample_count, sample_quality,
						EAH_GPU_Read | EAH_GPU_Write);
					pvp.merged_depth_ms_texs[i] = rf.MakeTexture2D(width, height, 1, 1, depth_fmt, sample_count, sample_quality,
						EAH_GPU_Read | EAH_GPU_Write);
				}
		...
		pvp.shading_ss_fb->Attach(FrameBuffer::ATT_Color0, rf.Make2DRenderView(*pvp.shading_ss_tex, 0, 1, 0));
		pvp.shading_ss_fb->Attach(FrameBuffer::ATT_DepthStencil, ss_ds_view);

		for (size_t i = 0; i < pvp.merged_shading_ss_texs.size(); ++ i)
		{
			pvp.merged_shading_ss_fbs[i]->Attach(FrameBuffer::ATT_Color0, rf.Make2DRenderView(*pvp.merged_shading_ss_texs[i], 0, 1, 0));
			pvp.merged_shading_ss_fbs[i]->Attach(FrameBuffer::ATT_DepthStencil, ss_ds_view);
			pvp.merged_depth_ss_fbs[i]->Attach(FrameBuffer::ATT_Color0, rf.Make2DRenderView(*pvp.merged_depth_ss_texs[i], 0, 1, 0));
			pvp.merged_depth_ss_fbs[i]->Attach(FrameBuffer::ATT_DepthStencil, ss_ds_view);
		}

		if (sample_count == 1)
		{
			pvp.shading_ms_fb = pvp.shading_ss_fb;

			for (size_t i = 0; i < pvp.merged_shading_ms_texs.size(); ++i)
			{
				pvp.merged_shading_ms_fbs[i] = pvp.merged_shading_ss_fbs[i];
				pvp.merged_depth_ms_fbs[i] = pvp.merged_depth_ss_fbs[i];
			}
		}
		else
		{
			pvp.shading_ms_fb->Attach(FrameBuffer::ATT_Color0, rf.Make2DRenderView(*pvp.shading_ms_tex, 0, 1, 0));
			pvp.shading_ms_fb->Attach(FrameBuffer::ATT_DepthStencil, ms_ds_view);

			for (size_t i = 0; i < pvp.merged_shading_ms_texs.size(); ++i)
			{
				pvp.merged_shading_ms_fbs[i]->Attach(FrameBuffer::ATT_Color0,
					rf.Make2DRenderView(*pvp.merged_shading_ms_texs[i], 0, 1, 0));
				pvp.merged_shading_ms_fbs[i]->Attach(FrameBuffer::ATT_DepthStencil, ms_ds_view);
				pvp.merged_depth_ms_fbs[i]->Attach(FrameBuffer::ATT_Color0,
					rf.Make2DRenderView(*pvp.merged_depth_ms_texs[i], 0, 1, 0));
				pvp.merged_depth_ms_fbs[i]->Attach(FrameBuffer::ATT_DepthStencil, ms_ds_view);
			}
		}
...
#endif
		if (viewports_[vp_index].sample_count != 1)
		{
			jobs_.push_back(MakeSharedPtr<DeferredRenderingJob>(
				[this, vp_index, pass_tb]
			{
				return this->GBufferGenerationDRJob(viewports_[vp_index], ComposePassType(PRT_MRT, pass_tb, PC_GBuffer), true);
			}));
		}
		jobs_.push_back(MakeSharedPtr<DeferredRenderingJob>(
			[this, vp_index, pass_tb]
			{
				return this->GBufferGenerationDRJob(viewports_[vp_index], ComposePassType(PRT_MRT, pass_tb, PC_GBuffer), false);
			}));
	...
	void DeferredRenderingLayer::GenerateMSGBuffer(PerViewport const & pvp, PassTargetBuffer pass_tb)
	{
		...
		if (PTB_Opaque == pass_tb)
		{
			...
			pvp.g_buffer_ms->GetViewport()->camera = camera;
#if DEFAULT_DEFERRED == TRIDITIONAL_DEFERRED
			pvp.lighting_fb->GetViewport()->camera = camera;
#endif
			pvp.vdm_fb->GetViewport()->camera = camera;
			pvp.shading_ss_fb->GetViewport()->camera = camera;
			pvp.shading_ms_fb->GetViewport()->camera = camera;
			for (size_t i = 0; i < pvp.merged_shading_ss_fbs.size(); ++ i)
			{
				pvp.merged_shading_ss_fbs[i]->GetViewport()->camera = camera;
				pvp.merged_depth_ss_fbs[i]->GetViewport()->camera = camera;

				pvp.merged_shading_ms_fbs[i]->GetViewport()->camera = camera;
				pvp.merged_depth_ms_fbs[i]->GetViewport()->camera = camera;
			}
		}

		re.BindFrameBuffer(pvp.g_buffer_ms);

		float depth = (PTB_TransparencyBack == pass_tb) ? 0.0f : 1.0f;
		int32_t stencil = (PTB_Opaque == pass_tb) ? 0 : 16;
		pvp.g_buffer_ms->Clear(FrameBuffer::CBM_Color | FrameBuffer::CBM_Depth | FrameBuffer::CBM_Stencil,
			Color(0, 0, 0, 0), depth, stencil);
	}

	void DeferredRenderingLayer::GenerateSSGBuffer(PerViewport const & pvp, PassTargetBuffer pass_tb)
	{
		RenderEngine& re = Context::Instance().RenderFactoryInstance().RenderEngineInstance();
		...
		{
			CameraPtr const & camera = pvp.frame_buffer->GetViewport()->camera;
			pvp.g_buffer_ss->GetViewport()->camera = camera;
#if DEFAULT_DEFERRED == TRIDITIONAL_DEFERRED
			pvp.lighting_fb->GetViewport()->camera = camera;
#endif
			pvp.vdm_fb->GetViewport()->camera = camera;
		...
		re.BindFrameBuffer(pvp.g_buffer_ss);

		float depth = (PTB_TransparencyBack == pass_tb) ? 0.0f : 1.0f;
		int32_t stencil = (PTB_Opaque == pass_tb) ? 0 : 16;
		pvp.g_buffer_ss->Clear(FrameBuffer::CBM_Color | FrameBuffer::CBM_Depth | FrameBuffer::CBM_Stencil,
			Color(0, 0, 0, 0), depth, stencil);
	}
	...
	void DeferredRenderingLayer::PostGenerateGBuffer(PerViewport const & pvp)
	{
		...
		if (cs_cldr_)
		{
			re.BindFrameBuffer(pvp.lighting_mask_ss_fb);
			re.CurFrameBuffer()->Attached(FrameBuffer::ATT_Color0)->ClearColor(Color(0, 0, 0, 0));
			re.Render(*dr_effect_, *technique_cldr_lighting_mask_[0], *rl_quad_);

			if (pvp.sample_count != 1)
			{
				re.BindFrameBuffer(pvp.lighting_mask_ms_fb);
				re.CurFrameBuffer()->Attached(FrameBuffer::ATT_Color0)->ClearColor(Color(0, 0, 0, 0));
				re.Render(*dr_effect_, *technique_cldr_lighting_mask_[1], *rl_quad_);
			}
	...
	void DeferredRenderingLayer::UpdateShadowingCS(PerViewport const & pvp)
	{
		...
		*lighting_mask_tex_param_ = pvp.lighting_mask_ss_tex;
	...
	void DeferredRenderingLayer::UpdateShading(PerViewport const & pvp, PassTargetBuffer pass_tb)
	{
		...
		*g_buffer_1_tex_param_ = pvp.g_buffer_resolved_rt1_tex;
	...
	void DeferredRenderingLayer::MergeIndirectLighting(PerViewport const & pvp, PassTargetBuffer pass_tb)
	{
		...
		{
			pvp.il_layer->CalcIndirectLighting(pvp.merged_shading_ms_texs[!pvp.curr_merged_buffer_index], pvp.proj_to_prev);
	...
	void DeferredRenderingLayer::MergeSSVO(PerViewport const & pvp, PassTargetBuffer pass_tb)
	{
		if (pvp.ssvo_enabled && !(pvp.attrib & VPAM_NoSSVO))
		{
			...
			ssvo_blur_pp_->OutputPin(0,
				(PTB_Opaque == pass_tb) ? pvp.merged_shading_ms_texs[pvp.curr_merged_buffer_index] : pvp.shading_ms_tex);
			ssvo_blur_pp_->Apply();
		}
	}
	...
	void DeferredRenderingLayer::AddTranslucency(uint32_t org_no, PerViewport const & pvp, PassTargetBuffer pass_tb)
	{
		...
		{
			auto* trans_pp = translucency_pp_[pvp.sample_count != 1].get();
			...
			if (light_camera)
			{
				trans_pp->OutputFrameBuffer()->Attach(FrameBuffer::ATT_DepthStencil,
					pvp.g_buffer_ms->Attached(FrameBuffer::ATT_DepthStencil));
				trans_pp->InputPin(0, pvp.g_buffer_ms_rt0_tex);
				trans_pp->InputPin(1, pvp.g_buffer_ms_rt1_tex);
				trans_pp->InputPin(2, pvp.g_buffer_ms_depth_tex);
	...
	{
		auto* sss_pp = sss_blur_pp_[pvp.sample_count != 1].get();

		sss_pp->OutputFrameBuffer()->Attach(FrameBuffer::ATT_DepthStencil,
			pvp.g_buffer_ms->Attached(FrameBuffer::ATT_DepthStencil));
		sss_pp->InputPin(0, pvp.merged_shading_ms_texs[pvp.curr_merged_buffer_index]);
		sss_pp->InputPin(1, pvp.g_buffer_ms_depth_tex);
		sss_pp->OutputPin(0, pvp.merged_shading_ms_texs[pvp.curr_merged_buffer_index]);
		sss_pp->Apply();
	}
	...
	void DeferredRenderingLayer::AddSSR(PerViewport const & pvp)
	{
		...
		re.BindFrameBuffer(pvp.merged_shading_ss_fbs[pvp.curr_merged_buffer_index]);
		ssr_pp_->InputPin(0, pvp.g_buffer_resolved_rt0_tex);
		ssr_pp_->InputPin(1, pvp.g_buffer_resolved_rt1_tex);
		ssr_pp_->InputPin(2, pvp.g_buffer_resolved_depth_tex);
		ssr_pp_->InputPin(3, pvp.merged_shading_ss_texs[!pvp.curr_merged_buffer_index]);
		ssr_pp_->InputPin(4, pvp.merged_depth_ss_texs[pvp.curr_merged_buffer_index]);
		ssr_pp_->Apply();
	}
	...
	void DeferredRenderingLayer::AddVDM(PerViewport const & pvp)
	{
		...
		vdm_composition_pp_->InputPin(3, pvp.merged_depth_ss_texs[pvp.curr_merged_buffer_index]);
	...
	void DeferredRenderingLayer::AddAtmospheric(PerViewport const & pvp)
	{
		...
		atmospheric_pp_->InputPin(0, pvp.merged_depth_ss_texs[pvp.curr_merged_buffer_index]);
	...
	void DeferredRenderingLayer::AddTAA(PerViewport const & pvp)
	{
		...
		pvp.frame_buffer->Discard(FrameBuffer::CBM_Color | FrameBuffer::CBM_Depth | FrameBuffer::CBM_Stencil);
		{
			if (pvp.sample_count == 1)
			{
				*depth_tex_param_ = pvp.merged_depth_ms_texs[pvp.curr_merged_buffer_index];
			}
			else
			{
				*depth_ms_tex_param_ = pvp.merged_depth_ms_texs[pvp.curr_merged_buffer_index];
			}
		...
		{
			taa_pp_[pvp.sample_count != 1]->InputPin(0, pvp.merged_shading_ms_texs[pvp.curr_merged_buffer_index]);
			taa_pp_[pvp.sample_count != 1]->InputPin(1, pvp.merged_shading_ms_texs[!pvp.curr_merged_buffer_index]);
			taa_pp_[pvp.sample_count != 1]->Render();
			re.Render(*dr_effect_, *technique_copy_depth_[pvp.sample_count != 1], *rl_quad_);
		}
		else
		{
			*shading_ms_tex_param_ = pvp.merged_shading_ms_texs[pvp.curr_merged_buffer_index];
			re.Render(*dr_effect_, *technique_copy_shading_depth_[pvp.sample_count != 1], *rl_quad_);
		}
	}
	...
	void DeferredRenderingLayer::MergeShadingAndDepth(PerViewport const & pvp, PassTargetBuffer pass_tb)
	{
		...
		if (pass_tb != PTB_Opaque)
		{
			...
			{
				re.BindFrameBuffer(pvp.merged_shading_ss_fbs[pvp.curr_merged_buffer_index]);
				*shading_tex_param_ = pvp.shading_ss_tex;
			}
			else
			{
				re.BindFrameBuffer(pvp.merged_shading_ms_fbs[pvp.curr_merged_buffer_index]);
				*shading_ms_tex_param_ = pvp.shading_ms_tex;
			}
			re.Render(*dr_effect_, *technique_merge_shadings_[(pvp.sample_count != 1) * 2 + (pass_tb != PTB_Opaque)], *rl_quad_);
		}

		if (pvp.sample_count == 1)
		{
			re.BindFrameBuffer(pvp.merged_depth_ss_fbs[pvp.curr_merged_buffer_index]);
			*depth_tex_param_ = pvp.g_buffer_resolved_depth_tex;
		}
		else
		{
			re.BindFrameBuffer(pvp.merged_depth_ms_fbs[pvp.curr_merged_buffer_index]);
			*depth_ms_tex_param_ = pvp.g_buffer_ms_depth_tex;
		}
		re.Render(*dr_effect_, *technique_merge_depths_[(pvp.sample_count != 1) * 2 + (pass_tb != PTB_Opaque)], *rl_quad_);
	}
	...
	void DeferredRenderingLayer::AccumulateToLightingTex(PerViewport const & pvp, PassTargetBuffer pass_tb)
	{
		...
		if (pvp.sample_count != 1)
		{
			if (PTB_Opaque == pass_tb)
			{
				pvp.merged_shading_ms_texs[pvp.curr_merged_buffer_index]->CopyToTexture(
					*pvp.merged_shading_ss_texs[pvp.curr_merged_buffer_index]);
			}
			else
			{
				pvp.shading_ms_tex->CopyToTexture(*pvp.shading_ss_tex);
			}
		}
	}
	...
	void DeferredRenderingLayer::CreateVDMDepthMaxMap(PerViewport const & pvp)
	{
		for (uint32_t i = 0; i < 2; ++ i)
		{
			TexturePtr input_tex = (0 == i) ? pvp.g_buffer_resolved_ds_tex : pvp.g_buffer_vdm_max_ds_texs[i - 1];
	...
	void DeferredRenderingLayer::UpdateLightIndexedLighting(PerViewport const & pvp, PassTargetBuffer pass_tb)
	{
		...
		*light_volume_mv_param_ = pvp.inv_proj;
		...
		RenderFactory& rf = Context::Instance().RenderFactoryInstance();
		RenderEngine& re = rf.RenderEngineInstance();
		re.BindFrameBuffer((PTB_Opaque == pass_tb) ? pvp.merged_shading_ss_fbs[pvp.curr_merged_buffer_index] : pvp.shading_ss_fb);
		re.Render(*dr_effect_, *technique_no_lighting_, *rl_quad_);
		...
		int32_t org_no, PassTargetBuffer pass_tb)
	{
		...
		*light_volume_mv_param_ = pvp.inv_proj;
		...
		{
			re.BindFrameBuffer(pvp.shading_ss_fb);
		}
		...
		std::vector<uint32_t>::const_iterator iter_beg, std::vector<uint32_t>::const_iterator iter_end)
	{
		...
		*min_max_depth_tex_param_ = pvp.g_buffer_min_max_depth_texs.back();
		...
		uint32_t w = (pvp.g_buffer_resolved_depth_tex->Width(0) + TILE_SIZE - 1) & ~(TILE_SIZE - 1);
		uint32_t h = (pvp.g_buffer_resolved_depth_tex->Height(0) + TILE_SIZE - 1) & ~(TILE_SIZE - 1);
		...
		*camera_proj_01_param_ = float2(pvp.proj(0, 0) * tile_scale.x(), pvp.proj(1, 1) * tile_scale.y());
		...
		w = pvp.g_buffer_resolved_depth_tex->Width(0);
		h = pvp.g_buffer_resolved_depth_tex->Height(0);
	...
	{
		uint32_t w = pvp.g_buffer_ms_depth_tex->Width(0);
		uint32_t h = pvp.g_buffer_ms_depth_tex->Height(0);
		depth_to_min_max_pp_->SetParam(0, float2(0.5f / w, 0.5f / h));
		depth_to_min_max_pp_->SetParam(1, float2(static_cast<float>((w + 1) & ~1) / w,
			static_cast<float>((h + 1) & ~1) / h));
		depth_to_min_max_pp_->InputPin(0, pvp.g_buffer_ms_depth_tex);
	...
	void DeferredRenderingLayer::UpdateClusteredLighting(PerViewport const & pvp, PassTargetBuffer pass_tb)
	{
		...
		*camera_proj_01_param_ = float2(pvp.proj(0, 0) * tile_scale.x(), pvp.proj(1, 1) * tile_scale.y());
		...
		else
		{
			*(dr_effect_->ParameterByName("g_buffer_ms_rt0_tex")) = pvp.g_buffer_ms_rt0_tex;
			*(dr_effect_->ParameterByName("g_buffer_ms_rt1_tex")) = pvp.g_buffer_ms_rt1_tex;
			*(dr_effect_->ParameterByName("g_buffer_ms_depth_tex")) = pvp.g_buffer_ms_depth_tex;
			*lighting_mask_ms_tex_param_ = pvp.lighting_mask_ms_tex;
		}
		...
		*skylight_diff_spec_mip_param_ = int3(0, 0, 0);
		...
		for (uint32_t li = 0; li < lights_.size();)
		{
			...
			*reinterpret_cast<uint32_t*>(lights_type + 0 * lights_type_param_->Stride()) = 0;
			...
			if (pvp.sample_count == 1)
			{
				if (pvp.temp_shading_ss_tex)
				{
					if (available_lights[0].empty())
					{
						*shading_in_tex_param_
							= (PTB_Opaque == pass_tb) ? pvp.merged_shading_ms_texs[pvp.curr_merged_buffer_index] : pvp.shading_ms_tex;
						*shading_rw_tex_param_ = pvp.temp_shading_ms_tex;
					}
					else
					{
			...
			else
			{
				if (pvp.temp_shading_ss_tex)
				{
					if (available_lights[0].empty())
					{
						*shading_in_ms_tex_param_
							= (PTB_Opaque == pass_tb) ? pvp.merged_shading_ms_texs[pvp.curr_merged_buffer_index] : pvp.shading_ms_tex;
					}
					else
					{
						*shading_in_ms_tex_param_ = TexturePtr();
					}
				}
			...
			*intersected_light_indices_in_tex_param_ = pvp.intersected_light_indices_tex;
			...
			if (pvp.sample_count != 1)
			{
				if (available_lights[0].empty() && pvp.temp_shading_ss_tex)
				{
					re.BindFrameBuffer(pvp.temp_shading_ms_fb);
				}
				else
				{
					re.BindFrameBuffer(
						(PTB_Opaque == pass_tb) ? pvp.merged_shading_ms_fbs[pvp.curr_merged_buffer_index] : pvp.shading_ms_fb);
				}
				*(dr_effect_->ParameterByName("src_2d_tex_array")) = pvp.temp_shading_tex_array;
				re.Render(*dr_effect_, *dr_effect_->TechniqueByName("UnflattenMultiSample"), *rl_quad_);

				// TODO: To be removed
				if (available_lights[0].empty() && pvp.temp_shading_ss_tex)
				{
					pvp.temp_shading_ms_tex->CopyToTexture(*pvp.temp_shading_ss_tex);
				}
				else
				{
					if (PTB_Opaque == pass_tb)
			...
			if (available_lights[0].empty() && pvp.temp_shading_ss_tex)
			{
				copy_pp_[0]->InputPin(0, pvp.temp_shading_ss_tex);
				copy_pp_[0]->OutputPin(0,
					(PTB_Opaque == pass_tb) ? pvp.merged_shading_ss_texs[pvp.curr_merged_buffer_index] : pvp.shading_ss_tex);
				copy_pp_[0]->Apply();

				copy_pp_[1]->InputPin(0, pvp.temp_shading_ms_tex);
				copy_pp_[1]->OutputPin(0,
					(PTB_Opaque == pass_tb) ? pvp.merged_shading_ms_texs[pvp.curr_merged_buffer_index] : pvp.shading_ms_tex);
				copy_pp_[1]->Apply();
			}
		}
	...
	void DeferredRenderingLayer::DumpIntermediaTextures()
	{
		...
		RenderFactory& rf = Context::Instance().RenderFactoryInstance();
		TexturePtr temp_tex = rf.MakeTexture2D(viewports_[0].g_buffer_ms->Width(), viewports_[0].g_buffer_ms->Height(),
	...
	uint32_t DeferredRenderingLayer::GBufferGenerationDRJob(PerViewport& pvp, PassType pass_type, bool multi_sample)
	{
		...
		*projective_shadowing_tex_param_ = pvp.projective_shadowing_tex;
		...
		if (multi_sample)
		{
			this->GenerateMSGBuffer(pvp, pass_tb);
		}
		else
		{
			this->GenerateSSGBuffer(pvp, pass_tb);
		}
	...
	uint32_t DeferredRenderingLayer::SpecialShadingDRJob(PerViewport& pvp, PassType pass_type)
	{
		...
		else
		{
			...
			{
				re.BindFrameBuffer(pvp.shading_ms_fb);
			}
		}
